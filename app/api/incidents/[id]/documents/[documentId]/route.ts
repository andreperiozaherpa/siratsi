...
export async function GET(_request: Request, { params }: { params: Promise<{ id: string; documentId: string }> }) {
  ...
  if (!(await canReadIncident(access, id))) return NextResponse.json({ error: 'Lampiran tidak ditemukan.' }, { status: 404 });
  const row = await env.DB!.prepare('SELECT object_key AS objectKey, content_type AS contentType FROM stakeholder_documents WHERE id = ? AND incident_id = ?').bind(documentId, id).first<{ objectKey: string; contentType: string }>();
