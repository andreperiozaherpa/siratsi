...
export async function GET(_request: Request, { params }: Context) {
  ...
  if (!incident || access.role === 'external_stakeholder') return NextResponse.json({ error: 'Informasi tidak ditemukan.' }, { status: 404 });
  const { results } = await env.DB!.prepare('SELECT id, filename, content_type AS contentType, size_bytes AS sizeBytes, created_at AS createdAt FROM approval_documents WHERE incident_id = ? ORDER BY created_at DESC').bind(id).all();
...
export async function POST(request: Request, { params }: Context) {
  try {
    ...
    if (incident.stage !== 3) return NextResponse.json({ error: 'Lampiran tersedia pada tahap Respons & Persetujuan.' }, { status: 409 });
    const approval = await env.DB!.prepare('SELECT status FROM incident_approvals WHERE incident_id=?').bind(id).first<{status:string}>();
    if (approval?.status === 'pending' || approval?.status === 'approved') return NextResponse.json({ error: 'Lampiran dikunci saat menunggu keputusan.' }, { status: 409 });
    ...
    const count = await env.DB!.prepare('SELECT COUNT(*) AS total FROM approval_documents WHERE incident_id = ?').bind(id).first<{ total: number }>();
    if ((count?.total || 0) >= MAX_FILES) return NextResponse.json({ error: 'Maksimal 5 lampiran per kasus.' }, { status: 400 });
    const documentId = crypto.randomUUID();
    const objectKey = `approval-documents/${id}/${documentId}`;
    await env.BUCKET.put(objectKey, bytes, { httpMetadata: { contentType: file.type } });
    try {
      await env.DB!.prepare('INSERT INTO approval_documents (id,incident_id,object_key,filename,content_type,size_bytes,uploaded_by,created_at) VALUES (?,?,?,?,?,?,?,?)')
